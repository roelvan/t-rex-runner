  ...
  Runner.prototype = {
    ...
    loadMidi() {
      WebMidi.enable(error => {
        if (error) {
          console.error('Midi could not be loaded...', error);
        }
        this.midiInput = WebMidi.getInputByName('Roland Digital Piano');
        if (this.midiInput) {
          // only when there is MIDI available
    ...
    onKeyDown: function(e) {
      ...
      if (e.target != this.detailsButton) {
        if (
          ...
          (Runner.keycodes.JUMP[e.keyCode] ||
            Runner.notes.JUMP[e.note && e.note.name] ||
      ...
      if (
        ...
        (Runner.keycodes.DUCK[e.keyCode] ||
          Runner.notes.DUCK[e.note && e.note.name])
      ) {
      ...
      if (this.isRunning() && isjumpKey) {
        ...
        if (
          ...
          (deltaTime >= this.config.GAMEOVER_CLEAR_TIME &&
            Runner.keycodes.JUMP[keyCode] || Runner.notes.JUMP[e.note && e.note.name])
        ) {
          this.restart();
        }
